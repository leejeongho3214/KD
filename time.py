    ...
    for epoch in range(250):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            for index in range(len(assistant_feature)):
                ...
                #   task-oriented feature distillation loss
                loss2 += torch.dist((teacher_feature[index]-student_feature2[index]), assistant_feature[index], p=2) * args.alpha
                #   task loss (cross entropy loss for the classification task)
                loss2 += criterion(assistant_outputs[index], labels)
                #   logit distillation loss, CrossEntropy implemented in utils.py.
                # loss2 += CrossEntropy(assistant_outputs[index],teacher_logits[index], 1 + (args.t / 250) * float(1 + epoch))
                # loss2 += torch.dist(assistant_feature[index], teacher_feature[index], p=2) * args.alpha
            ...
            _, predicted = torch.max(assistant_outputs[0].data+student_outputs2[0].data, 1)
            ...
            if i % 20 == 0:
                print('\r[epoch:%d, iter:%d] Loss: %.03f  Acc: %.2f%% '
        ...
        with torch.no_grad():
            ...
            for data in testloader:
                ...
                _, predicted2 = torch.max(student_outputs[0].data+0.1*assistant_outputs[0].data, 1)
                correct2 += float(predicted2.eq(labels.data).cpu().sum())


            if correct2 / total < best_acc:

                count += 1
                print('\r no no~~ %d Epoch   Acc: %.2f%% '
                      % ( epoch+1 ,100 * correct2 / total))
                if count == 50:
                    break
            else:
                count = 0
                best_acc = correct2 / total
                print("\r%d Epoch  acc: %.4f%% " % (epoch + 1, (100 * best_acc)))
                torch.save(assistant.state_dict(), "./checkpoint/assitant_" + args.model + "_2.pth")
